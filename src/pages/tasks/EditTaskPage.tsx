...
import type { Task } from "./TaskListPage";
import type { Department } from "../utils/responses/department";
import type { Staff } from "../staffs/StaffListPage"; // Make sure Staff is exported properly
import type { Programme } from "../programmes/ProgrammeLIstPage"; // Fixed typo: ProgrammeListPage
import type { Course } from "../courses/CourseListPage"; // Ensure Course type exists
...
export default function EditTaskPage() {
  ...
  const updateTask = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!id || !departmentId || !courseId || !staffId || !programmeId) {
    ...
    try {
      await CLIENT.put<Task>(`/tasks/${id}`, {
        title,
        description,
        dueDate,
        courseId,
